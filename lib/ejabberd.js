...
Ejabberd.prototype.initialize = function(cfgDir) {
  ...
  var dbFile = path.resolve(this.cfgDir, 'database.json');
  ...
  if (!fs.existsSync(dbFile)) {
    fs.writeFileSync(dbFile, JSON.stringify(EMPTY_DB), 'utf-8');
    this.db = EMPTY_DB;
  } else {
    this.db = require(dbFile);
  };
...
Ejabberd.prototype.addVhost = function(name, config) {
  var ejabberd = this;
  var file = path.resolve(ejabberd.incDir, ensureExt(name, 'cfg'));

  return config.updateVhostConfig(ejabberd, name, cfg).then(function() {
