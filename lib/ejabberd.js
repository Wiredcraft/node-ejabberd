...
// .
Ejabberd.prototype.updateVhosts = function(data, host, vCfg) {
  var ejabberd = this;
  var promises = [];
  var d = Q.defer();
  var db = EMPTY_DB;
  ...
  data.forEach(function(d, index) {
    db['hosts'].push(d.host);
    db['configs'].push(path.resolve(ejabberd.incDir, d.host + '.cfg'));
  });

  // Here, order matters
  promises.push(config.updateVhostConfig(ejabberd, host, vCfg));
  promises.push(config.updateConfig(ejabberd, db));
