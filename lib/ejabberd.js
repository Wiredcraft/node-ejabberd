...
Ejabberd.prototype.initialize = function(cfgDir, options) {
  ...
  ejabberd.checker = setInterval(function() {
    ejabberd.ping().then(
      ...
      if (options.enableCheckerLog) logger.info('Ejabber is running.');
    ...
    function() {
      ...
      if (options.enableCheckerLog) logger.error('Ejabberd is down.');
...
Ejabberd.prototype.addVhost = function(host, vCfg) {
  ...
  if (ejabberd.db['hosts'].indexOf(host) > -1) {
    return Q.fcall(function() { throw new Error('Host ' + host + ' is already existing'); });
  }
