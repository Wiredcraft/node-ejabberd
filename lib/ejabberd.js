...
Ejabberd.prototype.addVhost = function(host, vConfig) {
  vConfig = vConfig || {};
  vConfig.host = vConfig.host || host;
  ...
  ejabberd.db['includedConfigs'].push(file);

  return config.updateVhostConfig(ejabberd, host, vConfig).then(function() {
    return config.updateConfig(ejabberd, ejabberd.db['hosts'], ejabberd.db['includedConfigs']).then(function() {
      // .
      db.save(ejabberd);
